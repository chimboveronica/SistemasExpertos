...
function formatComandoPredefinido(val) {
    if (val === 'at$ttioco3=1') {
        return '<span style="color:green;">Bloquear Motor</span>';
    } else if (val === 'at$ttioco3=0') {
        return '<span style="color:green;">Desbloquear Motor</span>';
    } else if (val === 'at') {
        return '<span style="color:green;">COMPROBAR</span>';
    }
}
...
function formatorg(val) {
    if (val !== 'undefined') {
        ...
        return  val = '<span style="color: ' + record.get('color') + ';">' + record.get('text') + '</span>';
    }
}

//VARIABLES GLOBALES
function formatActiveUser(val) {
    var record = storeActiveUserList.getById(val);
    return '<span style="color: ' + record.get('color') + ';">' + record.get('text') + '</span>';
}

function formatEvento(val) {
   storeEventosReporte.reload();
 var record = storeEventosReporte.getById(val);
    return '<span style="color: ' + record.get('color') + ';">' + record.get('nombre') + '</span>';
...
function formatComp(val) {
    if (val === '<b>Total</b>') {
        ...
        var record = storeEmpresas.getById(val);
        val = '<span style="color: ' + record.get('color') + ';">' + record.get('text') + '</span>';
    }
    return val;
}
function formatRolUs(val) {
    return storRoles.getById(val).get('nombre');
}

function formatInvalid(val) {
    return storInvalid.getById(val).get('nombre');
}
...
function exportExcelEventos(grid, nameFile, nameSheet, titleSheet) {
            ...
            for (var i = 0; i < storeData.getCount(); i++) {
                ...
                for (var j = 0; j < columnsArray.length; j++) {
                    if (!columnsArray[j].renderer) {
                        ...
                        if (cadena[1] === 'formatSpeed(val)') {
                            table_div += "<Cell ss:StyleID='fTime'><Data ss:Type='DateTime'>" + formatSpeed(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatBat(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatBat(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'estadoGsm(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + estadoGsm(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'estadoGps(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + estadoGps(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatBatIgn(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatBatIgn(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'sinGSM(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + sinGSM(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        ...
                        if (cadena[1] === 'formatEstadoGeocerca(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatEstadoGeocerca(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatComandoPredefinido(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatComandoPredefinido(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        ...
                        if (cadena[1] === 'formatEstadoEquipo(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatEstadoEquipo(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatTipoServicio(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatTipoServicio(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatTipoRegistro(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatTipoRegistro(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatInvalid(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatInvalid(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatLock(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatLock(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                        if (cadena[1] === 'formatEvento(val)') {
                            table_div += "<Cell ss:StyleID='fString'><Data ss:Type='String'>" + formatEvento(storeData.data.items[i].get(columnsArray[j].dataIndex)) + "</Data></Cell>";
                        }
                    }
                }
                table_div += "</Row>";
