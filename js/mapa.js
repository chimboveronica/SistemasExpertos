        ...
        Ext.onReady(function () {
            ...
            map.events.register('click', map, function (e) {
                if (positionPoint) {
                    ...
                    console.log('latitud' + aux.y);
                    console.log('longitud' + aux.x);
                    winRuta.show();
                    positionPoint = false;
                }
            });
                ...
                eventListeners: {
                    featureselected: function (evt) {
                        ...
                        var contenidoAlternativo =
                                ...
                                "<b>Velocidad: </b>" + velocidad.toString() + "</br>" +
                                "<b>Direccion: </b>" + direccion.toString() + "</br>" +
                                "</section>";
                        var popup = new OpenLayers.Popup.FramedCloud("popup",
                                OpenLayers.LonLat.fromString(feature.geometry.toShortString()),
                                new OpenLayers.Size(200, 60),
...
//                        console.log('jjjj');
            ...
            lienzoPointRoute = new OpenLayers.Layer.Vector('Puntos de Ruta', {
                eventListeners: {
                    featureselected: function (evt) {
                        var feature = evt.feature;
                        var trama = feature.attributes.idTravel;
                        var placa = feature.attributes.placa;
                        var hora = feature.attributes.hora;
                        var direccion = feature.attributes.direccion;
                        var velocidad = feature.attributes.velocidad;
                ...
                eventListeners: {
                    featureselected: function (evt) {
                        ...
                        var contenidoAlternativo =
                                ...
                                "<b>Organización: </b>" + companyTravel.toString() + "<br>" +
            ...
            drawLineRuta = new OpenLayers.Control.DrawFeature(lines, OpenLayers.Handler.Path, {featureAdded: getRoute});
            modifyLineRuta = new OpenLayers.Control.ModifyFeature(lines);
            ...
            map.addControl(drawLineRuta);
            map.addControl(modifyLineRuta);
...
function onVehiculoSelect(evt) {
    ...
    var contenidoAlternativo =
            "<section>" +
            "<b>Organización: </b>" + companyLast + "<br>" +
...
function drawPointsPanicos(puntosPanico) {
    ...
    var yourLocation = new google.maps.LatLng(puntosPanico.latitud, puntosPanico.longitud);
    geocoder.geocode({'latLng': yourLocation}, function processGeocoder(results, status) {
        var features = new Array();
        ...
        var puntoMap = new OpenLayers.Feature.Vector(pt, {
            idTravel: puntosPanico.idData,
            idTrama: puntosPanico.idData,
            evento: puntosPanico.sky_evento,
...
function drawPointsParadas(puntosParadas) {
    var resultado;
    var geocoder = new google.maps.Geocoder();
    var yourLocation = new google.maps.LatLng(puntosParadas.latitud, puntosParadas.longitud);
    geocoder.geocode({'latLng': yourLocation}, function processGeocoder(results, status) {
//        resultado = results[0].formatted_address;
//        console.log(resultado);
        var features = new Array();
        var pt = new OpenLayers.Geometry.Point(puntosParadas.longitud, puntosParadas.latitud);
        pt.transform(new OpenLayers.Projection("EPSG:4326"),
                new OpenLayers.Projection("EPSG:900913"));
        var puntoMap = new OpenLayers.Feature.Vector(pt, {
            idTravel: puntosParadas.idData,
            idTrama: puntosParadas.idData,
            empresa: puntosParadas.empresa,
            vehiculo: puntosParadas.vehiculo,
            placa: puntosParadas.placa,
            bateria: puntosParadas.bateria,
            gps: puntosParadas.gps,
            gsm: puntosParadas.gsm,
            skyEvento: puntosParadas.sky_evento,
            velocidad: puntosParadas.velocidad,
            direccion: results[0].formatted_address,
            poppedup: false
        });
        puntoMap.id = 'route' + 0;
...
//        skyEvento: puntosParadas.sky_evento,
//        velocidad: puntosParadas.velocidad,
//        latitud: puntosParadas.latitud,
//        longitud: puntosParadas.longitud,
//        poppedup: false
//    });
//    puntoMap.id = 'route' + 0;
//    features.push(puntoMap);
...
//function drawPointsRoute(dataRuta) {
//    var geocoder = new google.maps.Geocoder();
//    var yourLocation = new google.maps.LatLng(dataRuta.latitud, dataRuta.longitud);
//    geocoder.geocode({'latLng': yourLocation}, function processGeocoder(results, status) {
//        var features = new Array();
//        var pt = new OpenLayers.Geometry.Point(dataRuta.longitud, dataRuta.latitud);
//        pt.transform(new OpenLayers.Projection("EPSG:4326"),
...
function clearLienzoPointTravel() {
    ...
    lienzoPointParadas.destroyFeatures();
    clearPopups();
    clearLienzoLineTravel();
    clearLienzoLineRoute();
    clearLienzoPointRouteManual();
...
function getRoute(fig) {
    var listPoint = fig.geometry.getVertices();
    console.log(listPoint.length);
    drawRouteMapa = false;
    drawLineRuta.deactivate();
    Ext.getCmp('btnedit').setIconCls("icon-act-mapa");
     Ext.getCmp('btnedit').enable();
    winRuta.show();
    var auxPointsRoute = getDataOfFeature(listPoint);
    console.log(auxPointsRoute);
}