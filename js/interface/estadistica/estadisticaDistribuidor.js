...
Ext.onReady(function () {
    ...
    storeCountAccessByUser = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: 'php/interface/estadistica/estadisticaDistribuidor/getCountAccessByUser.php',
            reader: {
                type: 'json',
                root: 'data'
            }
        ...
        listeners: {
            load: function (thisObject, records, successful, eOpts) {
                if (records.length > 0) {
                    var pos_ini = records[0].data.total;
                    var mayor = pos_ini;
                    for (var i = 0; i < records.length; i++) {
                        var dat = records[i].data.total;
                        if (dat > pos_ini) {
                            mayor = dat;
                        }
                    }
    ...
    storeCountExcesosVelocidad = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: 'php/interface/estadistica/estadisticaDistribuidor/getCountExcesosVelocidad.php',
            reader: {
                type: 'json',
                root: 'data'
            }
    ...
    storeCountByDevice = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: 'php/interface/estadistica/estadisticaDistribuidor/getCountByDevice.php',
            reader: {
                type: 'json',
                root: 'data'
            }
        },
        fields: ['idEquipo', 'reg_municipal', 'ultimaTrama', 'total'],
        listeners: {
            load: function (thisObject, records, successful, eOpts) {
                if (records.length > 0) {
                    var pos_ini = records[0].data;
    ...
    storeCountTypeByDevice = Ext.create('Ext.data.Store', {
        proxy: {
            type: 'ajax',
            url: 'php/interface/estadistica/estadisticaDistribuidor/getCountTypeByDevice.php',
            reader: {
                type: 'json',
                root: 'data'
            }
        ...
        series: [{
                ...
                tips: {
                    ...
                    renderer: function (storeItem, item) {
                        this.setTitle("<center>Información</center>");
                        this.update(
                                "<b>Placa:</b> " + storeItem.get('reg_municipal') +
